 ...
 * subscription to one of them based on a hash of the pair. Each inner transport
 * opens its own WebSocket connection. This sidesteps any per-connection
 * subscription cap on the data provider side — BlockSize Capital closes
 * connections that have more than ~85 concurrent vwap subscriptions after
 * ~60s.
 *
 * The inner transports all share the same response cache, so consumers of the
 * cache don't need to know which shard owns which pair.
 */
